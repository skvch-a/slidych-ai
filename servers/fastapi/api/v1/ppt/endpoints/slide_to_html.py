...
async def generate_html_from_slide_google(
    base64_image: str,
    xml_content: str,
    api_key: str,
    model: str,
    ...
    """
    Generate HTML content from slide image and XML using Google Gemini API.
    """
    ...
    try:
        ...
        if not html_content:
            raise HTTPException(
                status_code=500, detail="No HTML content generated by Google Gemini"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Google API error during HTML generation: {error_msg}",
        )


async def generate_html_from_slide_openai(
    base64_image: str,
    media_type: str,
    xml_content: str,
    api_key: str,
...
async def generate_html_from_slide(
    ...
    """
    Generate HTML content from slide image and XML using configured LLM provider.
    """
    config = get_user_config()

    if config.LLM == "google" and config.GOOGLE_API_KEY and config.GOOGLE_MODEL:
        return await generate_html_from_slide_google(
        ...
        return await generate_html_from_slide_openai(
            base64_image=base64_image,
        ...
        raise HTTPException(
            status_code=500,
            detail="No API key configured. Please configure OpenAI or Google API key in settings.",
        )


async def generate_react_component_from_html_google(
    html_content: str,
    api_key: str,
    model: str,
    ...
    """
    Convert HTML content to TSX React component using Google Gemini API.
    """
    ...
    try:
        ...
        if not react_content:
            raise HTTPException(
                status_code=500, detail="No React component generated by Google Gemini"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Google API error during React generation: {error_msg}",
        )


async def generate_react_component_from_html_openai(
    html_content: str,
    api_key: str,
    image_base64: Optional[str] = None,
...
async def generate_react_component_from_html(
    ...
    """
    Convert HTML content to TSX React component using configured LLM provider.
    """
    config = get_user_config()

    if config.LLM == "google" and config.GOOGLE_API_KEY and config.GOOGLE_MODEL:
        return await generate_react_component_from_html_google(
        ...
        return await generate_react_component_from_html_openai(
            html_content=html_content,
            api_key=config.OPENAI_API_KEY,
            image_base64=image_base64,
            media_type=media_type,
...
async def convert_slide_to_html(request: SlideToHtmlRequest):
    ...
    try:
        ...
        # Generate HTML using the configured LLM provider
...
async def convert_html_to_react(request: HtmlToReactRequest):
    ...
    try:
        ...
        # Convert HTML to React component using configured LLM provider
